...
```{r}
multi_logical <- grepl(".*/.*", as.character(StormData_mod$EVTYPE), ignore.case = T)
heat_logical <- grepl(".*HEAT.*", as.character(StormData_mod$EVTYPE), ignore.case = T)
names_heat <- unique(as.character(StormData_mod[!multi_logical & heat_logical, "EVTYPE"]))
print(names_heat)
```

```{r}
head(StormData_mod[StormData_mod$EVTYPE %in% names_heat,])
```
