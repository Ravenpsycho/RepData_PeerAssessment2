...
```{r}
testmatch_fld <- grepl("FLD", 
                       as.character(StormData_mod$EVTYPE), 
                       ignore.case = T)
levels(factor(StormData_mod[testmatch_fld, "EVTYPE"]))

testmatch_non <- grepl("NON-? ?\\w",
                       as.character(StormData_mod$EVTYPE), 
                       ignore.case = T, perl =T)
levels(factor(StormData_mod[testmatch_non, "EVTYPE"]))

testmatch_tstm <- grepl("TSTM", 
                        as.character(StormData_mod$EVTYPE), 
                        ignore.case = T)
levels(factor(StormData_mod[testmatch_tstm, "EVTYPE"]))
...
matches <- amatch(toupper(StormData_mod$EVTYPE), my_str_short, method = "lcs", maxDist = 14)
df_test_amatch <- data.frame(EVTYPE = StormData_mod$EVTYPE, 
                             RESULT = my_str_short[matches])
sum(is.na(df_test_amatch$RESULT))
amatch_10 <- which(is.na(df_test_amatch$RESULT))
df_test_amatch[amatch_10,]
```
