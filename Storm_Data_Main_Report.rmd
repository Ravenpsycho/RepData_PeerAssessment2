...
```{r}
multi_logical <- grepl(".*/.*", as.character(StormData_mod$EVTYPE),
                       ignore.case = T)
heat_logical <- grepl(".*HEAT.*", as.character(StormData_mod$EVTYPE),
                      ignore.case = T)
heat_single <- !multi_logical & heat_logical
names_heat <- unique(as.character(StormData_mod[heat_single, "EVTYPE"]))
print(names_heat)
```

```{r}
StormData_mod$EVTYPE[heat_single] <- names_heat[1]
level_nb <- which(levels(StormData_mod$EVTYPE) == names_heat[1])
levels(StormData_mod$EVTYPE)[level_nb] <- "HEAT COMBINED"
unique(as.character(StormData_mod[heat_single, "EVTYPE"]))
```
...
flood_logical <- grepl("^ *(FLOOD */)? *(FLASH|)+ *FLOOD(S)*(ING)*$", as.character(StormData_mod$EVTYPE),
                      ignore.case = T)
# flood_single <- !multi_logical & flood_logical
...
```{r}
plotpop <- head(df_by_event[order(desc(df_by_event$index)),], 20)
plotpop$EVTYPE <- factor(plotpop$EVTYPE, levels = as.character(plotpop$EVTYPE))

prettycolors <- colorRampPalette(c("darkred", "gold3"))
...
g1 + geom_bar(stat = "identity", position = "dodge") +
        ...
        scale_fill_manual(values = prettycolors(20))+
...
prettycolors <- colorRampPalette(c("darkgreen", "springgreen"))
g2 <- ggplot(plotpop2[c(1:6)],
             aes(x=EVTYPE, y = index, fill = EVTYPE))
g2 + geom_bar(stat = "identity", position = "dodge") +
        ylab("Cost issues index")+
        xlab("Event")+
        theme(legend.position = "none")+
        scale_fill_manual(values = prettycolors(20))+
        theme(axis.text.x = element_text(angle = 45, hjust = 0.95)) +
        labs(title = paste0("Top 20 events in decreasing order of Cost issues",
                            " index (CII).\n CII = mean of propriety damage +",
                            " mean of crop damage ",
                            "* total occurences\nData for ",
                            "years 1950 to 2011"))
