...
rm(my_pdf, my_str)
my_str_long <- unlist(strsplit(my_str_short, "[/]"))
print(my_str_short)
...
```{r}
patterns <- c("FLD", "NON-? ?\\w*", "TSTM",
              "URBAN/SML STREAM", "Ice jam flood [(]minor",
              "MIXED PRECIPITATION", "HYPOTHERMIA", "TORRENTIAL RAINFALL",
              "HYPERTHERMIA", "AND LIGHTNING$", "HWY", "EXTREME")
replacements <-  c("FLOOD", "", "THUNDERSTORM", "", "FLOOD", "RAIN",
                   "COLD", "RAIN", "HEAT", "", "HEAVY", "")
...
matches1 <- amatch(toupper(as.character(StormData_mod$EVTYPE)), my_str_long, method = "lcs", maxDist = 8)
matches2 <- amatch(toupper(my_str_long[matches1]), my_str_short, method = "lcs", maxDist = 8)

df_test_amatch <- data.frame(EVTYPE = StormData_mod$EVTYPE, 
                             RESULT = my_str_long[matches1],
                             RESULT2 = my_str_short[matches2])
sum(is.na(df_test_amatch$RESULT2))
amatch_7 <- which(is.na(df_test_amatch$RESULT))
df_test_amatch[amatch_7,]
```
